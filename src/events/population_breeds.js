...
export function populationBreeds(settlement, random = Math.random) {
  ...
  return settlement
    ...
    .updateIn(['logs', settlement.get('turn')], (currentEvents) =>
      currentEvents.concat(newPeopleAndLogs.get('logs')),
    )
}
...
function coupleBreeds(couple, random) {
  ...
  let child = makePerson({
    age: 0,
    parents: Immutable.Set.of(id(father), id(mother)),
  })

  return Immutable.fromJS({
    people: [
      child,
      father.update('children', Immutable.List(), (children) =>
        children.push(id(child)),
      ),
      mother.update('children', Immutable.List(), (children) =>
        children.push(id(child)),
      ),
    ],
    logs: [
