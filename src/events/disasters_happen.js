...
const DISASTER_TYPES = ['flood', 'fire']
...
export function disastersHappen(settlement, random = Math.random) {
  ...
  return settlement
    ...
    .updateIn(['logs', settlement.get('turn')], (currentEvents) =>
      currentEvents.push(disasterLog),
    )
}
...
function killPeople(people, random) {
  ...
  return people.reduce((acc, person) => {
    if (random() > chanceToDie) {
      return acc.update('newPeople', (newPeople) =>
        newPeople.set(id(person), person),
      )
    }
