...
export function populationMarries(settlement, random = Math.random) {
  ...
  let alivePeople = people.filter(
    (settler) =>
      !isDead(settler) && age(settler) >= config.MINIMUM_AGE_TO_MARRY,
  )
...
function pairUpSingles(singlesMap, random) {
  ...
  return pairs
    ...
    .filter((pair) => {
      return (
        Math.abs(age(pair.first()) - age(pair.last())) <=
          config.MAX_AGE_DIFFERENCE_TO_MARRY &&
